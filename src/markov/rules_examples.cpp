...
void ExtendTrunkRule::apply(Chunk& C, const vec3& pos) const {
	...
	if (rand() % 7 == 0 || z >= MAX_TRUNK_HEIGHT) { // Randomly decide to extend the trunk
		C.m_pBlocks[x][y][z].block_type = BlockType_Wood; // Set the current block to Wood
    }
    else {
...
int build_tree_aux(Chunk& C, int step) {
    ...
    case 1:
        if (BuildGroundGrass.applyRule(C, 1)) {
            break;
        }
        step = 2;
        break;
    ...
    case 2:
        x_t = rand() % (C.CHUNK_SIZE / 2) + C.CHUNK_SIZE / 4;
        y_t = rand() % (C.CHUNK_SIZE / 2) + C.CHUNK_SIZE / 4;
        z_t = 1;
        C.m_pBlocks[x_t][y_t][z_t].block_type = BlockType_Wood_Tmp;
        std::cout << "Set the head at " << x_t << " " << y_t << " " << z_t << std::endl;
        step = 3;
        break;
