...
OldRule left_disapear = OldRule({vec3(0, 0, 0), vec3(-1, 0, 0)},
                                {
                                    BlockType_Default,
                                    BlockType_Empty,
                                },
...
OldRule build_base_from_xm =
    OldRule({vec3(0, 0, 0), vec3(-1, 0, 0)},
            {BlockType_Empty, BlockType_Default}, BlockType_Default, 1.0f);

OldRule build_base_from_xp =
    OldRule({vec3(0, 0, 0), vec3(1, 0, 0)},
            {BlockType_Empty, BlockType_Default}, BlockType_Default, 1.0f);

OldRule build_base_from_ym =
    OldRule({vec3(0, 0, 0), vec3(0, -1, 0)},
            {BlockType_Empty, BlockType_Default}, BlockType_Default, 1.0f);

OldRule build_base_from_yp =
    OldRule({vec3(0, 0, 0), vec3(0, 1, 0)},
            {BlockType_Empty, BlockType_Default}, BlockType_Default, 1.0f);

OldRule build_from_under =
    OldRule({vec3(0, 0, 0), vec3(0, 0, -1)},
            {BlockType_Empty, BlockType_Default}, BlockType_Default, 1.0f);

OldRule appear_OldRule =
    OldRule({vec3(0, 0, 0)}, {BlockType_Empty}, BlockType_Default, 1.0f);

OldMarkovRule keep_only_full = OldMarkovRule({left_disapear});
OldMarkovRule build_base =
    OldMarkovRule({build_base_from_xm, build_base_from_xp, build_base_from_ym,
                   build_base_from_yp});
